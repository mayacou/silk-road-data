...
def build_country_centroid_reference():
    ...
    # --- VISUAL CENTROID OVERRIDES ---
    # Centers massive countries visually rather than mathematically
    MANUAL_CENTROIDS = {
        "USA": {"centroid_lon": -98.58, "centroid_lat": 39.82},  # Central USA
        "CAN": {"centroid_lon": -95.00, "centroid_lat": 55.00},  # Lower Canada
        "RUS": {"centroid_lon": 90.00,  "centroid_lat": 60.00},  # Central Russia
        "FRA": {"centroid_lon": 2.21,   "centroid_lat": 46.22},  # Metropolitan France
    }
    for iso, coords in MANUAL_CENTROIDS.items():
        if iso in country_ref:
            country_ref[iso] = coords
            
    # --- SAVE TO JSON FOR FRONTEND & EXPORT ---
    centroids_file = os.path.join(BASE_DIR, "data", "country_centroids.json")
    with open(centroids_file, "w") as f:
        json.dump({k: {"lat": v["centroid_lat"], "lon": v["centroid_lon"]} for k, v in country_ref.items()}, f, indent=2)
...
def calculate_total_emissions(net_weight_kg, distance_km, emission_factor):
    """
    Calculates total emissions using the formula:
    total_emissions = net_weight_kg * distance_km * emission_factor
    
    Args:
        net_weight_kg: Weight in kilograms
        distance_km: Distance in kilometers
        emission_factor: kg CO2e per kg per km
        
    Returns:
        Total emissions in kg CO2e, or None if inputs are invalid
    """
    if net_weight_kg is None or distance_km is None or emission_factor is None:
        return None
    return net_weight_kg * distance_km * emission_factor


def calculate_emissions_per_kg(total_emissions_kgco2e, net_weight_kg):
    """
    Calculates emissions per unit weight.
    
    Args:
        total_emissions_kgco2e: Total emissions in kg CO2e
        net_weight_kg: Weight in kilograms
        
    Returns:
        Emissions per kg (kg CO2e / kg), or None if inputs are invalid
    """
    if total_emissions_kgco2e is None or net_weight_kg is None or net_weight_kg == 0:
        return None
    return total_emissions_kgco2e / net_weight_kg
...
def enrich_year(year: str, country_ref: dict):
    ...
    df["total_emissions_kgco2e"] = df.apply(
        lambda row: calculate_total_emissions(row["netWgt"], row["distance_km"], row["emission_factor_kgco2e_per_kg_km"]),
        axis=1
    )
    df["emissions_per_kg"] = df.apply(
        lambda row: calculate_emissions_per_kg(row["total_emissions_kgco2e"], row["netWgt"]),
        axis=1
    )
